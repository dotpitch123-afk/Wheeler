...
//       Vehicle.find({ user_id: String(userId), is_active: true }).lean(),
...
const getHomeDashboard = async (req, res) => {
  try {
    ...
    console.log("recentBookings:", recentBookings.map(b => b.vehicle_id));
    ...
    return res.status(200).json({
      ...
      data: {
        user: {
          name: user?.first_name || user?.name?.split(" ")[0] || "User",
        },
        active_booking:  activeBookingData,
        vehicles:        resolvedVehicles,
        recent_bookings: recentBookingsData,
